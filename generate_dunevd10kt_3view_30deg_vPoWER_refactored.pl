...
$PoWER_LDU_side = 20.0;     # Lado da LDU quadrada
$PoWER_LDU_thick = 2.0;    # Espessura total da unidade

$PoWER_Veto_side = 5.0;     # Lado da região central de Veto (VUV)
...
-->
  ...
  <!-- PROPRIEDADES DE EMISSÃO DO WLS -->
   <matrix name="PEN_WLS_COMPONENT" coldim="2" values=" 2.0*eV 0.1  2.5*eV 0.5  3.0*eV 1.0"/>

   <!-- PROPRIEDADES DE ABSORÇÃO DO WLS -->
   <matrix name="PEN_WLS_ABSLENGTH" coldim="2" values="2.0*eV 100*cm 9.0*eV 0.01*cm"/>
   

  <!-- LDU e VETO -->

  <!-- Matriz de eficiência para LDU -->
  <matrix name="PoWER_EFFICIENCY_HIGH" coldim="2" values=" 2.0*eV 0.10 2.3*eV 0.25 2.6*eV 0.55 3.0*eV 0.75    3.3*eV 0.60 3.6*eV 0.35 4.0*eV 0.15 4.5*eV 0.05"/>

  <!-- Matriz de eficiência  para Veto -->
  <matrix name="PoWER_EFFICIENCY_LOW" coldim="2" values=" 2.0*eV 0.03 2.5*eV 0.10 3.0*eV 0.23    3.5*eV 0.15 4.0*eV 0.05"/>
...
my $asmix = <<EOF;
  ...
  <material name="PEN" formula="C14H10O4">
    <D value="1.30" unit="g/cm3"/>
    <fraction n="0.76" ref="carbon"/>
    <fraction n="0.05" ref="hydrogen"/>
    <fraction n="0.19" ref="oxygen"/>
  </material>

    <!-- Definição do Silicio  -->
    ...
    <!-- Material do encapsulamento (epoxy) -->
    <material name="Epoxy" formula="C21H25ClO5">
        <D value="1.2" unit="g/cm3"/>
        <composite n="21" ref="carbon"/>
        <composite n="25" ref="hydrogen"/>
        <composite n="1" ref="chlorine"/>
        <composite n="5" ref="oxygen"/>
    </material>

    <!-- Material do SiPM (silício) -->
    <material name="Si_Epoxy" formula="Si_Epoxy">
      <D value="1.85" unit="g/cm3"/>
      <fraction n="0.60" ref="Silicon"/>
...
sub gen_PWR() {
    ...
    # Dimensões do LDU e do Veto
    my $sideLDU  = $PoWER_LDU_side; 
    my $thickLDU = $PoWER_LDU_thick;
    my $sideVeto = $PoWER_Veto_side;

    $PWR = $basename."_PWR" . $suffix . ".gdml";
    push (@gdmlFiles, $PWR);
    $PWR = ">" . $PWR;
    open(PWR) or die("Could not open file $PWR for writing");
...
print $PWR <<EOF;
<solids>
    <box name="PoWER_LDU_Solid" lunit="cm" 
         x="$sideLDU" y="$sideLDU" z="$thickLDU"/>
    <box name="PoWER_Veto_Solid" lunit="cm" 
         x="$sideVeto" y="$sideVeto" z="$thickLDU"/>

      <!-- Superfície óptica para o LDU -->
  <opticalsurface name="SurfacePoWER_LDU" model="unified" finish="polished" type="photocathode">
      <property name="EFFICIENCY" ref="PoWER_EFFICIENCY_HIGH"/>
      <property name="REFLECTIVITY" value="0.05"/>
  </opticalsurface>

  <!-- Superfície óptica para o Veto -->
  <opticalsurface name="SurfacePoWER_Veto" model="unified" finish="polished" type="photocathode">
      <property name="EFFICIENCY" ref="PoWER_EFFICIENCY_LOW"/>
  </opticalsurface>
</solids>
EOF
...
<structure>
    ...
    <!-- Volume do Veto - região central de 5x5 cm -->
    <volume name="volPoWERVeto">
        <materialref ref="Si_Epoxy"/>
        <solidref ref="PoWER_Veto_Solid"/>
        <auxiliary auxtype="SensDet" auxvalue="Veto"/>
        <colorref ref="red"/>
    </volume>

    <!-- Volume do LDU - região de 20x20 cm -->
    <volume name="volPoWERLDU">
        <materialref ref="Si_Epoxy"/>
        <solidref ref="PoWER_LDU_Solid"/>

        <physvol name="physPoWERVeto">
            <volumeref ref="volPoWERVeto"/>
            <position name="posVetoInLDU" x="0" y="0" z="0" unit="cm"/>
        </physvol>

        <auxiliary auxtype="SensDet" auxvalue="LDU"/>
        <colorref ref="lightblue"/>
    </volume>

    <!-- Superfície óptica para o LDU -->
    <skinsurface name="PoWER_LDU_Skin" surfacepoint="SurfacePoWER_LDU">
        <volumeref ref="volPoWERLDU"/>
    </skinsurface>

    <!-- Superfície óptica para o Veto -->
    <skinsurface name="PoWER_Veto_Skin" surfacepoint="SurfacePoWER_Veto">
        <volumeref ref="volPoWERVeto"/>
    </skinsurface>
...
my $lengthPEN = $lengthTPCActive;
my $widthPEN = $widthTPCActive;
my $thickPEN = 0.01; # cm, just for visualization purposes


$PEN = $basename."_PEN" . $suffix . ".gdml";
push (@gdmlFiles, $PEN);
$PEN = ">" . $PEN;
open(PEN) or die("Could not open file $PEN for writing");
...
<solids>
    <box name="BoxPEN" lunit="cm" x="$thickPEN" y="$widthPEN" z="$lengthPEN"/>
    
    <opticalsurface name="SurfacePEN" model="unified" finish="polished" type="dielectric_dielectric" value="1.0">

      <!-- Diz ao Geant4 qual é o espectro de luz emitida quando o PEN é excitado -->
      <property name="WLSCOMPONENT" ref="PEN_WLS_COMPONENT"/>

      <!-- Diz ao Geant4 com que eficiência o PEN absorve fótons de diferentes energias -->
      <property name="WLSABSLENGTH" ref="PEN_WLS_ABSLENGTH"/>

      <!-- NÚMERO MÉDIO DE FÓTONS EMITIDOS POR FÓTON ABSORVIDO -->
      
      <property name="WLSMEANUMBER" value="0.85"/>

      <!-- EFICIÊNCIA QUÂNTICA DO WLS -->
      
      <!-- Eficiência quântica do processo de conversão -->
      <property name="WLSQUANTUMEFFICIENCY" value="0.9"/>

      <!-- Tempo médio entre a absorção de um fóton e a emissão de um novo fóton -->
      <property name="WLSTIMECONSTANT" value="1.6*ns"/>

      <!-- Tempo de decaimento do estado excitado -->
      <property name="WLSDELAY" value="0.1*ns"/>

      <!-- Fração da luz que é refletida pela superfície -->
      <property name="REFLECTIVITY" value="0.95"/>

      <!-- SOMA DA REFLETIVIDADE TOTAL (0.5 + 0.45 + 0 + 0) --> 

      <!-- DIFFUSELOBECONSTANT é definido implicitamente 0.45 -->
      <property name="SPECULARLOBECONSTANT" value="0.5"/>
      <property name="SPECULARSPIKECONSTANT" value="0.0"/>
      <property name="BACKSCATTERCONSTANT" value="0.0"/>
...
<structure>
    ...
    <skinsurface name="PEN_Skin" surfacepoint="SurfacePEN">
        <volumeref ref="volPEN"/>
    </skinsurface>
</structure>
...
EOF
    ...
    print "Generated PEN fragment: $PEN\n";
}
...
#<!-- PEN covers -->
#<skinsurface name="PEN_Cover_Standard" surfacepoint="SurfacePEN">
#    <volumeref ref="volFieldShaper"/>
#</skinsurface>
#<skinsurface name="PEN_Cover_Slim" surfacepoint="SurfacePEN">
...
sub place_FieldShaper()
{
    ...
    my $posX;
    my $posZ;
    my $posY = -0.5 * $FieldShaperShortTubeLength - $FieldShaperTorRad;
    my $thickPEN = 0.2; 
    
    
    
    my $PEN_posY = $posY - 0.01 - 0.5*$thickPEN; # place the PEN cover 1 mm away from the field shaper, and account for its thickness
...
print CRYO <<EOF;
    <physvol>
      <volumeref ref="volFieldShaperSlim"/>
      <position name="posFieldShaper_$reversed_$i" unit="cm"  x="$posX" y="$posY" z="0" />
...
print CRYO <<EOF;
  <physvol>
      <volumeref ref="volPEN"/>
      <position name="posPEN_${reversed}_$i" unit="cm" x="$posX" y="$PEN_posY" z="$posZ" />
    </physvol>
EOF
}
...
gen_PWR();      # generates PoWER materials and surfaces
gen_PEN();       # generates PEN materials and surfaces
